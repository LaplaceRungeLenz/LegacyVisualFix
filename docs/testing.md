# LegacyVisualFix 测试记录

## 2026-09-16 快捷栏选择框层级（0.7.2）

Forge 原本先画选择框、后画物品；此前只修改选择框坐标，移动中仍会被物品遮住。现将启用动效的选择框延后到九个物品及其覆盖层之后、HOTBAR Post 事件之前绘制，临时关闭深度测试并恢复 GL 状态，继续使用资源包中的 widgets 纹理。禁用效果保留原版绘制顺序。

实际快捷栏回归检查在旧代码下因“选择框必须在九个物品后只画一次”失败，修复后通过；同时检查绘制时深度测试关闭、平滑插值、实际选中即时生效及禁用回退。Java 8 与 Java 25 + Angelica 2.1.25 / lwjgl3ify 3.0.10（beta3 相关依赖）客户端 UI 测试均 PASS。README 精简为功能、安装、配置和许可，不再链接测试报告。此次仅交付 JAR，不创建 Release。

## 2026-09-15 0.7.1 发布包回归核对

- 用户实例仅有 `legacyvisualfix-0.7.0.jar`，SHA-256 为 `c977bd47145aeaa1397aa2d7520a61117b461ef4380f0aa44e63e0a081ff09b1`，与首次交付包完全相同。字节码缺少 `InventoryScreenEvents` 和 `InventoryMotion.openingFrom`，输入仍是旧 Redirect 写法，未包含随后合并的 0.6.3 修复。GitHub main（7e932dc）与本地修复源码已包含这些修复；此次不是源代码再次丢失修复，而是旧产物以相同版本号交付造成混淆。
...
- 修复前：日志记录 ModularUI2 鼠标/键盘 Redirect 被 LegacyVisualFix 抢占，真实 `GuiScreen.handleInput` 左键拿取测试失败；只有 NEI、MouseTweaks 的组合不复现。返回新建背包对象的测试也失败。
...
烟雾测试报告位于 `run/client/legacyvisualfix-inventory-smoke/result.txt`；客户端会捕获断言后退出，必须检查报告的 `PASS`，不能只看 Gradle 退出码。输入和返回路径测试均不编入正常交付 JAR。
...
交付构建 `spotlessApply clean build --no-configuration-cache` 通过：**14 项 JUnit 测试**、Spotless、Checkstyle、重混淆打包。`legacyvisualfix-0.4.0.jar` 已确认包含两份 Mixin 配置和 refmap，主类字节码版本 52（Java 8），不含 smoke 测试类或 Waila / Chromatic 依赖类。SHA-256：`a9f322e2d50394728deb98dd6d51b7f89322b9b3d24ad8d534aa0e645e7512e8`。
...
本机 Java 直连 GitHub 超时，Chromatic 官方 dev JAR 经系统网络下载后由 `-PchromaticRepository=file:///C:/Users/rog/Documents/ChatGPT/LegacyVisualFix/.gradle/chromatic-artifacts/` 读取。仓库默认仍使用官方 GitHub URL，正常 JAR 不包含这两个依赖。
...
发布包 `legacyvisualfix-0.3.0.jar` 已检查：包含默认 PNG、配置、Mixin 配置与 refmap，主类版本为 52（Java 8），不包含 `ClientSmoke` 或测试模组。SHA-256：`2b017f9fd397d5a59ad179072b5e56db7d9f911a567dd096fd2dd082bbe1f98c`。
...
故意失败用例会产生监听器异常和原版资源包回退日志。缺失图片用例会产生 LegacyVisualFix 回退警告。原版 Forge 的联网版本检查和快速连续重载的异步音频系统也可能输出错误；需和 LegacyVisualFix 的绘制错误分别判断。

## 人工验收清单

1. 进入语言选择界面，切换英文/中文，检查加载结束后返回界面且文字正确。
2. 启用/禁用大型资源包，再按 F3+T；确认阶段变化且最后退出加载界面。
3. 修改 `config/legacyvisualfix/theme.properties` 和本地 PNG，检查下次重载生效。
...
复现：`runClient -PuiSmoke -PneiSmoke -PuiMuiSmoke -PuiNeiVersion=2.8.130-GTNH`；Angelica 使用 `runClient25 -PuiSmoke -PneiSmoke -PuiMuiSmoke -PangelicaSmoke`。必须检查 `LEGACYVISUALFIX_UI_SMOKE PASS` 与两个 `LEGACYVISUALFIX_UI_MUI_SMOKE PASS`，不能仅用 Gradle 退出成功判断效果测试通过。
