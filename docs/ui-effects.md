...
五项客户端效果：快捷栏选中框平滑移动；悬停物品缩放；鼠标携带物品缩放与阻尼倾斜；相同物品浮动；普通物品的银白色拖尾及其他稀有度物品的彩色拖尾。
...
仅悬停时是平滑放大；阻尼晃动发生在拿起物品后移动鼠标时。槽位浅色高亮沿用原版绘制，本身不会阻止点击。

拖尾使用物品真正的 `getRarity()` 及其颜色，不依据 GT 电压等级或名称颜色推断。1.7.10 的普通下界之星虽然闪光，仍属于 common，因此使用银白色拖尾；带附魔的稀有物品可以触发。粒子按移动距离和时间发射，停止移动后消失，长停顿不会补发旧路径。
