...
@Mod(
    modid = "legacyvisualfixsmoke",
    name = "LegacyVisualFix smoke test",
    version = "1",
...
public class ClientSmoke {
    ...
    public void tick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END && waitingForFade) {
                    ...
                    try {
                        screenshot(
                            new File(
                                ...
                                "legacyvisualfix-smoke/fade-frame-" + exitTicks + ".png"));
                    } catch (Exception e) {
            ...
            File results = new File(mc.mcDataDir, "legacyvisualfix-smoke");
            try {
                ...
                System.out.println("LEGACYVISUALFIX_SMOKE_PASS");
            } catch (Throwable e) {
                e.printStackTrace();
                try {
                    Files.write(
        ...
        results.mkdirs();
        try {
            File themeFile = new File(mc.mcDataDir, "config/legacyvisualfix/theme.properties");
            try (java.io.InputStream defaults = getClass()
                .getResourceAsStream("/assets/legacyvisualfix/theme-0.1.properties")) {
            ...
            for (reloads = 0; reloads < 5; reloads++) {
                if (reloads == 1) {
                    try (java.io.InputStream logo = getClass()
                        .getResourceAsStream("/assets/legacyvisualfix/textures/gui/logo.png")) {
                        Files.copy(
                            logo,
                    ...
                    File theme = new File(mc.mcDataDir, "config/legacyvisualfix/theme.properties");
                    Files.write(
                        theme.toPath(),
                ...
                if (reloads == 3) {
                    File pack = new File(mc.mcDataDir, "resourcepacks/LegacyVisualFix-Smoke");
                    File gui = new File(pack, "assets/legacyvisualfix/textures/gui");
                    gui.mkdirs();
                    Files.write(
                        new File(pack, "pack.mcmeta").toPath(),
                        "{\"pack\":{\"pack_format\":1,\"description\":\"LegacyVisualFix smoke pack\"}}"
                            .getBytes(StandardCharsets.UTF_8));
                    ...
                    for (ResourcePackRepository.Entry entry : repository.getRepositoryEntriesAll()) {
                        if ("LegacyVisualFix-Smoke".equals(entry.getResourcePackName())) {
            ...
            if (((com.legacyvisualfix.reload.ReloadProgress) progressField.get(null)).isActive()) {
