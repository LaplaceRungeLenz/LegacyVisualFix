...
Minecraft **1.7.10 / Forge 10.13.4.1614** 客户端模组。切换语言、应用资源包、按 F3+T，以及 Angelica 光影启用、关闭、更换和重新加载时显示可定制的加载界面。进入和退出具有淡入淡出效果。

基于 [GTNH Project Starter](https://github.com/GTNewHorizons/ExampleMod1.7.10) 和 GTNHGradle 构建。默认主题采用略提亮的深蓝渐变、GTNH Jappa 风格重绘 Logo、扁平金色进度条和简洁状态文字。背景和进度条由本项目生成；Logo 来源及独立许可见下文。

![默认加载界面](docs/screenshots/default.png)

## 安装

1. 将 `build/libs/modernnh-0.3.0.jar` 放入客户端 `mods/`。
...
版本在 `addon.gradle` 中定义为 `0.3.0`，可用环境变量 `VERSION` 覆盖。上游 `build.gradle.kts` 保持原样，依赖放在 `dependencies.gradle`。
...
冒烟测试会修改**开发客户端**的配置和测试资源包，保存截图与 `modernnh-smoke/result.txt` 后退出；Angelica 环境另写入 `shader-result.txt`。测试包含真实最小光影包的切换和程序编译。它只在显式使用 `-PclientSmoke` 时编译；**不要携带该参数构建发布包**。发布前执行 `gradlew clean build`，确保没有测试模组残留。

## 许可

代码及原创背景、进度条、像素字形采用 MIT，见 [LICENSE](LICENSE)。GTNH Jappa 风格重绘 Logo 独立适用 CC BY-NC-SA 4.0，来源、署名和许可见 [美术资源说明](src/main/resources/META-INF/NOTICE-GTNH.md)。使用 GTNH 开发工具不表示 GTNH 官方认可或维护本项目。
